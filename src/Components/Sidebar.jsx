...
import { IoHomeOutline, IoClose, IoBriefcase } from "react-icons/io5";
import { IoIosPerson } from "react-icons/io";
import { BsSuitcaseLgFill } from "react-icons/bs";
import { IoLayersOutline } from "react-icons/io5";
import { LuMessagesSquare } from "react-icons/lu";
import photo from "../assets/my.jfif";
	...
	return (
		<div className={`fixed top-0 left-0 h-screen bg-[#081328] p-8 z-50 transform lg:translate-x-0 ${menuOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out w-full lg:w-1/5 lg:flex lg:flex-col lg:justify-between lg:items-center border-r border-[#5B5A6A] flex flex-col justify-between items-center`}>
			...
			<div className='flex flex-col items-center justify-center gap-3'>
				<img className='h-40 rounded-full' src={photo} />
				...
				<ul className='space-y-2 my-2 text-white text-sm lg:text-md'>
					<li className='flex items-center justify-center gap-[6px]'> <span><IoBriefcase size={'18px'} className="text-yellow-color" /></span> Siemens Energy</li>
					<li className='flex items-center justify-center gap-[6px]'> <span><IoLocationOutline size={'18px'} className="text-yellow-color" /></span> Pune, Maharashtra, India</li>
